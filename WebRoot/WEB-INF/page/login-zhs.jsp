...
<html>
	  ...
	  <div class="pwd-page">
	    <div class="login-frame" id="find_pwd">
	      <div class="top-frame">
	    	...
	    	<span style="font-size:20px;margin-bottom:10px">找回密码</span>
	      </div>
	      <form role="form" id="loginFrom" name="loginFrom" method="post" action="login.do">
   		    <div class="form-group" style="padding:0 25px">
      	      ...
      	      <input type="text" class="form-control" id="username_F" name="username" autocomplete="off" placeholder="用户名" required="required"/>
   		    </div>
     	      ...
     	      <input type="text" style="display:none;"/>
      	      <input type="text" class="form-control" style="width:185px" id="password_F" name="N_PASSWORD_F" autocomplete="off" onfocus="this.type='password'" onBlur="if(this.value==null||this.value==''){this.type='text'}" placeholder="新密码" required="required"/>
   		      <input type="text" style="display:none;"/>
              ...
              <i class="fa fa-eye pointer left hide" data-pwd="hide" data-frame="password_F"></i>
   		    ...
   		    <div  class="form-group" style="padding:0 25px">	   
   		      ...
   		      <input type="text" id="textMsg_F" name="textMsg" style="width:120px"  class="form-control textMsg left" autocomplete="off" placeholder="验证码" required="required"/>
   		  	  <input id="getMessage_F" type="button" class="btn-op pointer" value="获取验证码"/>
   		  	</div>
   		    <input type="hidden" id="lang" name="lang" value="ZHS"/>
			<input id="confirm" type="button" class="btn pointer" data-crudtype="update" data-pageframe="find_pwd" data-updateurl="forgetPwd.do" data-func="$().findPWD();" value="修 改 密 码"/>
			<input type="button" data-select="goback" class="forget-pwd pointer" value="返回登录页面"/>
	    </form>
	    </div>
	  </div>
	</div>
  ...
  <script>
    ...
    $.fn.findPWD = function(){
        RegExpValidate('^[a-zA-Z0-9]{5,14}$','password_F','$("#password_F").val("");regExpError("密码格式错误，必须是由字母或数字组成的6-15位字符");');
    }
  </script>
