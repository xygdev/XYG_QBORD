  ...
  <body>
    <div id="container">
      ...
      <div id="scrollbar" class="table pointer">
        <table  id="main-table" data-table="Perm">
          <tr> 
            ...
            <th class="USER_DESC" data-column="db">Name</th>
          ...
          <tr>
            ...
            <td class="ACTION" data-column="normal">       
              <i class="fa fa-trash fa-fw pointer hidden" data-show="true" title="Delete" data-refresh="refresh"  data-col="USER_NAME" data-crudtype="del" data-delurl="perm/deleteInvPermission.do" data-delmsg="Delete User Account：" data-delparam=["PERMISSION_ID",".PERMISSION_ID"] ></i>       
            </td>
      ...
      <div id="query" class="pop_frame row-3">
        ...
        <div class="content row-3">
          <form>
            ...
            <br style="clear:both"/>
            <label for="START_DATE_ACTIVE_F" class="left md">Start Date:</label>
            <input type="text" id="START_DATE_ACTIVE_F" name="START_DATE_ACTIVE_F" class="left lg" data-datatype="date" placeholder="Start Date From"/>
            <label class="left blank"></label>
            <input type="text" id="START_DATE_ACTIVE_T" name="START_DATE_ACTIVE_T" class="left lg" data-datatype="date" placeholder="Start Date To"/>
          </form>     
        </div>
        <div class="foot">
      ...
      <div id="ui" class="pop_frame row-2" >
        <div class="title pointer">
          <span data-type="insert"><i class="fa fa-unlock-alt fa-1x" aria-hidden="true"></i>&nbsp;Insert</span>
        </div>
        ...
        <div class="content row-2">
          <form id="updateData">  
            ...
            <input type="button" id="USER_LOV" class="left button pointer" data-pageframe="lov" data-reveal-id="lov"  data-key="true" data-callback="ui" data-bg="lov-modal-bg" data-dismissmodalclass="close-lov" data-lovname="User Query" data-queryurl="lov/getUserPage.do" data-jsontype="user" data-defaultquery="true" data-th=["id","Account","Name"] data-td=["USER_ID&none","USER_NAME","DESCRIPTION"] data-selectname=["Account","Name"] data-selectvalue=["USER_NAME","DESCRIPTION"] data-choose=[".USER_ID",".USER_NAME",".DESCRIPTION"] data-recid=["#U_ID","#USER_NAME","#DESCRIPTION"] value="···"/>
            <label for="DESCRIPTION" class="left md">Name</label> 
            <input type="text" id="DESCRIPTION" name="DESCRIPTION" data-update="db" class="left lg" readonly="readonly" required="required"/>         
            <label for="ORGANIZATION_CODE" class="left md">Code</label> 
            ...
            <label for="ORGANIZATION_NAME" class="left md">Inv</label>
        ...
        <div class="foot">
          <button class="right update_confirm pointer" data-type="insert" data-keyup="enter" data-crudtype="insert" data-pageframe="ui" data-inserturl="perm/insertInvPermission.do">Insert</button>
        </div>
      </div>
      <!-- 更新/新增用户区域 end -->
      <!-- 用户信息存放区域 start -->
	            ...
	            $('[data-enable]').on('click',function(){
	                ...
	                $.ajax({
		                ...
		                success:function(data){
			                if(data.retcode=='0'){
			                    if(action == 'OFF'){
			                        layer.msg('Disable Succeed!');
			                    }else if(action == 'ON'){
			                        layer.msg('Enable Succeed!');
			                    }
			                    $('#refresh').click();/****点击刷新当前页按钮，刷新数据****/  
			                }else{
			                    layer.alert('Update Failed！Error Message:'+data.errbuf,{title:'警告',offset:[150]});
			                }
	                    },error:function(){
